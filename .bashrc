...
alias l="less"
alias sudo='sudo ' # for aliases to work with sudo
alias s="sudo"
alias se="s -E"
alias zs="se $SHELL"
alias rr="rm -rf"
...
__git_local_head() {
	...
	COMPREPLY=( $(compgen -W "$(ls ${gr}/.git/refs/remotes/origin)" -- $cur) )
}

complete -F __git_local_head gbrd

git-prepare-patches() {
	sed -i 's/^From:.*/From: Daniel Martí <mvdan@mvdan.cc>/' *.patch
}
...
logcat() {
	adb logcat | grep `adb shell ps | grep $1 | sed 1q | cut -c10-15`
}
